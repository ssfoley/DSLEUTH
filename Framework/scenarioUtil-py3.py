...
class ScenarioUtil:
    ...
    def __init__(self, scen_file_name, dest_path, pieces, log_file):
        ...
        self.original.print_me(self.log_file)
        print ("Scenario File: \n")
        ...
        print("pieces: {} -- numper {} -- combos {}".format(pieces, numper, combos), file=self.log_file)
        print("diffNum {} -- breedNum {} -- spreadNum {} -- slopeNum {} -- roadNum {}".format(orig.diffNum, orig.breedNum, orig.spreadNum, orig.slopeNum, orig.roadNum), file=self.log_file)
    ...
    def gen_scen_objs(self, sel_cfg, log_file):
        sel_cfg_key = "".join([str(x) for x in sel_cfg[:5]])
        if sel_cfg_key in self.dict_scen_obj:
            return self.gen_scen_route(self.dict_scen_obj[sel_cfg_key])
        else:
            print("OOPS!!!!", file=log_file)
            print(sel_cfg, file=log_file)
            return []
        
    def gen_scen_route(self, parameters):
        if len(parameters) == 1:
            return self.gen_scens_one_param(parameters[0])
        elif len(parameters) == 2:
            return self.gen_scens_two_param(parameters[0], parameters[1])
        elif len(parameters) == 3:
            return self.gen_scens_three_param(parameters[0], parameters[1], parameters[2])
        else:
            print("OOPS!!!!", file=self.log_file)
            return []
        
    def gen_scens_one_param(self, inputName):
        scens = []
        orig = self.original
    ...
    def gen_scens_two_param(self, inputName1, inputName2):
    ...
    def gen_scens_three_param(self, inputName1, inputName2, inputName3):
    ...
    def pick_best_config(self, poss_config):
        ...
        print("selected_config: {}".format(selected_config))
...
if __name__ == "__main__":
    scen_util = ScenarioUtil("./scenario.demo200-calibrate", "./scenarioUtil_output", 4,"scenarioUtil.log")
    print(scen_util.scen_file_list)
