...
class Scenario:
    ...
    def print_me(self, log_file):
        print("diffusion: {} {} {}".format(
            self.diffStart, self.diffStep, self.diffStop), file=log_file)
        print("breed:  {} {} {}".format(
            self.breedStart, self.breedStep, self.breedStop), file=log_file)
        print("spread:  {} {} {}".format(
            self.spreadStart, self.spreadStep, self.spreadStop), file=log_file)
        print("slope:  {} {} {}".format(
            self.slopeStart, self.slopeStep, self.slopeStop), file=log_file)
        print("road:  {} {} {}".format(
            self.roadStart, self.roadStep, self.roadStop), file=log_file)
    ...
    def write_file(self, orig_file, dest_file, fnum):
        ...
        for line in lines:
            ...
            if line is not "" and line[0] != '#':
                if self.diffusionStartPattern in line:
                    # found diffusion start
                    df.write(self.diffusionStartPattern +
                             str(self.diffStart) + "\n")
                elif self.diffusionStepPattern in line:
                    # found diffusion step
                    df.write(self.diffusionStepPattern +
                             str(self.diffStep) + "\n")
                elif self.diffusionStopPattern in line:
                    # found diffusion stop
                    df.write(self.diffusionStopPattern +
                             str(self.diffStop) + "\n")
                elif self.breedStartPattern in line:
                    # found breed start
                    df.write(self.breedStartPattern +
                             str(self.breedStart) + "\n")
                elif self.breedStepPattern in line:
                    # found breed step
                    df.write(self.breedStepPattern +
                             str(self.breedStep) + "\n")
                elif self.breedStopPattern in line:
                    # found breed stop
                    df.write(self.breedStopPattern +
                             str(self.breedStop) + "\n")
                elif self.spreadStartPattern in line:
                    # found spread start
                    df.write(self.spreadStartPattern +
                             str(self.spreadStart) + "\n")
                elif self.spreadStepPattern in line:
                    # found spread step
                    df.write(self.spreadStepPattern +
                             str(self.spreadStep) + "\n")
                elif self.spreadStopPattern in line:
                    # found spread stop
                    df.write(self.spreadStopPattern +
                             str(self.spreadStop) + "\n")
                elif self.slopeStartPattern in line:
                    # found slope start
                    df.write(self.slopeStartPattern +
                             str(self.slopeStart) + "\n")
                elif self.slopeStepPattern in line:
                    # found slope step
                    df.write(self.slopeStepPattern +
                             str(self.slopeStep) + "\n")
                elif self.slopeStopPattern in line:
                    # found slope stop
                    df.write(self.slopeStopPattern +
                             str(self.slopeStop) + "\n")
                elif self.roadStartPattern in line:
                    # found road start
                    df.write(self.roadStartPattern +
                             str(self.roadStart) + "\n")
                elif self.roadStepPattern in line:
                    # found road step
